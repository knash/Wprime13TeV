...
for coup in ['right','left','mixed']:
	...
	Data= ROOT.TFile("rootfiles/TBanalyzerdata_Trigger_none_none_PSET_"+cuts+".root")

	TTmc = ROOT.TFile("rootfiles/TBanalyzerttbar_Trigger_nominal_none_PSET_"+cuts+".root")
	...
	TTmcBup = TTmc.Get("MtbBup")

	TTmcBdown = TTmc.Get("MtbBDown")

	TTmcptup = TTmc.Get("Mtbptup")
	
	TTmcptdown = TTmc.Get("Mtbptdown")
	...
	for RA in range(0,len(mass)):
		...
		for i in range(0,len(modsup)):
			...
			SignalupB11 = ROOT.TFile("rootfiles/TBanalyzerweightedsignal"+coup+str(mass[RA])+"_Trigger_"+trigsup[i]+"_"+modsup[i]+"_PSET_"+cuts+".root")
			SignaldownB11 = ROOT.TFile("rootfiles/TBanalyzerweightedsignal"+coup+str(mass[RA])+"_Trigger_"+trigsdown[i]+"_"+modsdown[i]+"_PSET_"+cuts+".root")
			SignalFSup = SignalupB11.Get("Mtb")
			SignalFSdown = SignaldownB11.Get("Mtb")
			output.cd()

			SignalFSup.Rebin(rebin)
			SignalFSdown.Rebin(rebin)

			SignalFSup.SetTitle("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"up")
			SignalFSdown.SetTitle("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"down")

			SignalFSup.SetName("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"up")
			SignalFSdown.SetName("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"down")

			SignalFSup.Write("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"up")
			SignalFSdown.Write("mtb_allhad__wp"+str(mass[RA])+LabelsU[i]+"down")
